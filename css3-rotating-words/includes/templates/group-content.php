<h3 class="tab-head">
    <?php echo esc_html( !empty($data['rw_group_name']) ? $data['rw_group_name'] : esc_html__('Rotating Words', 'la-wordsrotator') ); ?>
    <button title="<?php esc_attr_e('Get Shortcode For This Group of Words', 'la-wordsrotator'); ?>" class="button-primary fullshortcode" id="<?php echo esc_attr($data['counter']); ?>">
                ...
                <td style="width:30%">
                    <?php if ( isset( $field['placeholder'] ) ) : ?>
                        <input class="<?php echo esc_attr( $field['input_class'] ); ?>" 
                               ...
                               value="<?php echo esc_attr( !empty($data[$field['field']]) ? $data[$field['field']] : esc_html__( 'Default Text', 'la-wordsrotator' ) ); ?>">
                    <?php else : ?>
                        <select class="<?php echo esc_attr( $field['input_class'] ); ?>">
                            <?php foreach ($field['options'] as $value => $label): ?>
                                <?php $selected = ($data[$field['field']] == $value) ? 'selected="selected"' : ''; ?>
                                <option value="<?php echo esc_attr( $value ); ?>" <?php selected( $data['animation_effect'], $value ); ?>><?php echo esc_html( $label ); ?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php endif; ?>
                </td>
